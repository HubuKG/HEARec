...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        # ------------------------------------------------------------------------------------------------------------------
        self.warm_missing_item_index = self._compute_warm_missing_item_index()

    def _compute_warm_missing_item_index(self):
        # 确保 dataset.df 是一个 pandas DataFrame
        df = self.dataset.df  # 假设 dataset.df 是一个 pandas DataFrame
        if isinstance(df, pd.DataFrame):
            # 使用 groupby 和 size 计算每个 itemID 的评分次数
            item_counts = df.groupby('itemID').size()

            cold_start_threshold = 35  # 设置冷启动项的阈值  15       Baby阈值为10，Sports目前为25, Clothing目前为35
            cold_start_items = item_counts[item_counts < cold_start_threshold].index.tolist()

            return cold_start_items
        else:
            raise ValueError("self.dataset.df is not a pandas DataFrame")

    def get_warm_missing_item_index(self):
        return self.warm_missing_item_index
